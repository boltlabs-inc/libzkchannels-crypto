...
mod test {
    ...
    */
    macro_rules! test_commitment_mod {
        ($group:path, [$length:literal], $name:ident) => {
            #[cfg(test)]
            paste! {
            #[test]
            fn [<$name _commit_open_success_len$length >]() {
                let mut rng = crate::test::rng();
                let params = PedersenParameters::<$group, $length>::new(&mut rng);
                let msg = Message::random(&mut rng);
                let bf = BlindingFactor::new(&mut rng);
            ...
            #[test]
            fn [<$name _ commit_does_not_open_on_wrong_msg_len$length >]() {
                let mut rng = crate::test::rng();
                let params = PedersenParameters::<$group, $length>::new(&mut rng);
                let msg = Message::random(&mut rng);
                let bf = BlindingFactor::new(&mut rng);
            ...
            #[test]
            fn [<$name _ commit_does_not_open_on_wrong_bf_len$length >]() {
                let mut rng = crate::test::rng();
                let params = PedersenParameters::<$group, $length>::new(&mut rng);
                let msg = Message::random(&mut rng);
                let bf = BlindingFactor::new(&mut rng);
            ...
            #[test]
            fn [<$name _ commit_does_not_open_on_wrong_commit_len$length >]() {
                let mut rng = crate::test::rng();
                let params = PedersenParameters::<$group, $length>::new(&mut rng);
                let msg = Message::random(&mut rng);
                let bf = BlindingFactor::new(&mut rng);
            ...
            #[test]
            fn [<$name _ commit_does_not_open_on_random_commit_len$length >]() {
                let mut rng = crate::test::rng();
                let params = PedersenParameters::<$group, $length>::new(&mut rng);
                let msg = Message::random(&mut rng);
                let bf = BlindingFactor::new(&mut rng);

                let bad_com = Commitment::<$group>($group::random(&mut rng));
            ...
            #[cfg(feature = "bincode")]
            #[test]
            fn [<$name _ serialize_deserialize_pedersen_params_len$length >]() {
                let mut rng = crate::test::rng();
                let params = PedersenParameters::<$group, $length>::new(&mut rng);

                // Check normal serialization/deserialization
                let ser_params = bincode::serialize(&params).unwrap();
                let new_params = bincode::deserialize::<PedersenParameters<$group, $length>>(&ser_params).unwrap();
                assert_eq!(params, new_params);

                // Check validation when h in the Pedersen parameters is the identity element
                let mut bad_params = PedersenParameters::<$group, $length>::new(&mut rng);
                bad_params.h = $group::identity();
                let ser_params = bincode::serialize(&bad_params).unwrap();
                assert!(bincode::deserialize::<PedersenParameters<$group, $length>>(&ser_params).is_err());
                ...
                let mut gs = bad_params.gs.to_vec();
                let last_position = gs.len() - 1;
                gs[last_position] = $group::identity();
                bad_params.gs = Box::try_from(gs.into_boxed_slice()).unwrap();
                let ser_params = bincode::serialize(&bad_params).unwrap();
                assert!(bincode::deserialize::<PedersenParameters<$group, $length>>(&ser_params).is_err());
                ...
                let mut gs = bad_params.gs.to_vec();
                let random_position = rng.gen_range(0..gs.len());
                gs[random_position] = $group::identity();
                bad_params.gs = Box::try_from(gs.into_boxed_slice()).unwrap();
                let ser_params = bincode::serialize(&bad_params).unwrap();
                assert!(bincode::deserialize::<PedersenParameters<$group, $length>>(&ser_params).is_err());
            }
        ...
        ($group:path, [$length:literal, $($rest:literal),*], $name:ident) => {
            test_commitment_mod!($group, [$length], $name);
            test_commitment_mod!($group, [$($rest),*], $name);
        };
    }
